...
```
    ...
    │                         │
    │   GPU0        GPU1      │  ← PCIe slots (e.g., dual-GPU NVLink)
    │                         │
...
2. Edit the target IP if needed (default: `192.0.2.137`)
3. Run manually or install as scheduled task:

```powershell
# Test run
.\gpu-temp-sender.ps1 -TargetIP "192.0.2.137"
...
Test-NetConnection 192.0.2.137 -Port 9999
```
...
*Built for the Mac Pro 2019 running Proxmox with multi-GPU passthrough.*

## VM Agent (GPU temperature sender)

The host can't see GPU temps once cards are passed through to a VM — the
guest reports them via UDP. Linux guest: `bash install-vm-agent.sh`
(script + systemd unit). Windows guest (legacy): `gpu-temp-sender.ps1`.
Architecture, config, and verification: [docs/VM-AGENT.md](docs/VM-AGENT.md).
